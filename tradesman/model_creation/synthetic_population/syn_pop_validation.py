...
def validate_non_controlled_vars(model_place:str, folder:str):
    ...
    controls = pd.read_csv("tradesman/model_creation/synthetic_population/controls_and_validation/hh_composition_data_validation.csv")
...
def uniformity(validation_dir, seed_hh_cols, popsim_dir, seed_hh_file, exp_hh_file, exp_hh_id_col):
    ...
    for i, g in enumerate(geogs):
        ...
        fig, ax = geog_fig.add_subplot(1, len(geogs), i+1)
    ...
    fig.savefig(os.path.join(validation_dir, 'expansion_factor_range.pdf'))
